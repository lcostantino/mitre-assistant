...
pub struct EnterpriseMatrixBreakdown {
    ...
    pub enterprise_revoked_techniques: EnterpriseRevokedTechniques,
...
impl EnterpriseMatrixParser {
    ...
    fn extract_deprecated_techniques(
        ...
        if items["x_mitre_deprecated"].as_bool().unwrap() {
            ...
            self.details.stats.count_deprecated_techniques =
                self.details.deprecated_techniques.len();
        } else {
            self.extract_techniques_and_tactics(items, false);
        }
    ...
    fn extract_stats_techniques_by_platforms(&mut self, _wants_subtechniques: bool) {
        ...
        if _wants_subtechniques {
            ...
            self.details.stats.percent_subtechniques_azure =
                self.get_percentage(_total, _azure.len());
            self.details.stats.percent_subtechniques_azure_ad =
                self.get_percentage(_total, _azure_ad.len());
            self.details.stats.percent_subtechniques_gcp = self.get_percentage(_total, _gcp.len());
            self.details.stats.percent_subtechniques_linux =
                self.get_percentage(_total, _linux.len());
            self.details.stats.percent_subtechniques_macos =
                self.get_percentage(_total, _macos.len());
            self.details.stats.percent_subtechniques_office365 =
                self.get_percentage(_total, _office365.len());
            self.details.stats.percent_subtechniques_saas =
                self.get_percentage(_total, _saas.len());
            self.details.stats.percent_subtechniques_windows =
                self.get_percentage(_total, _windows.len());
        } else {
            ...
            self.details.stats.percent_techniques_azure_ad =
                self.get_percentage(_total, _azure_ad.len());
            ...
            self.details.stats.percent_techniques_office365 =
                self.get_percentage(_total, _office365.len());
            self.details.stats.percent_techniques_saas = self.get_percentage(_total, _saas.len());
            self.details.stats.percent_techniques_windows =
                self.get_percentage(_total, _windows.len());
        }
    }
    ///
    ...
    fn extract_stats_techniques_by_killchain(&mut self, _wants_subtechniques: bool) {
        ...
        if _wants_subtechniques {
            ...
            self.details.stats.percent_subtechniques_credential_access =
                self.get_percentage(_total, _credential_access.len());
            self.details.stats.percent_subtechniques_collection =
                self.get_percentage(_total, _collection.len());
            self.details.stats.percent_subtechniques_discovery =
                self.get_percentage(_total, _discovery.len());
            self.details.stats.percent_subtechniques_lateral_movement =
                self.get_percentage(_total, _lateral_movement.len());
            self.details.stats.percent_subtechniques_command_and_control =
                self.get_percentage(_total, _command_and_control.len());
            self.details.stats.percent_subtechniques_exfiltration =
                self.get_percentage(_total, _exfiltration.len());
            self.details.stats.percent_subtechniques_impact =
                self.get_percentage(_total, _impact.len());
            // Rollup
            ...
            self.details.stats.percent_techniques_execution =
                self.get_percentage(_total, _execution.len());
            self.details.stats.percent_techniques_persistence =
                self.get_percentage(_total, _persistence.len());
            self.details.stats.percent_techniques_privilege_escalation =
                self.get_percentage(_total, _priv_escalation.len());
            self.details.stats.percent_techniques_defense_evasion =
                self.get_percentage(_total, _defense_evasion.len());
            self.details.stats.percent_techniques_credential_access =
                self.get_percentage(_total, _credential_access.len());
            self.details.stats.percent_techniques_collection =
                self.get_percentage(_total, _collection.len());
            self.details.stats.percent_techniques_discovery =
                self.get_percentage(_total, _discovery.len());
            self.details.stats.percent_techniques_lateral_movement =
                self.get_percentage(_total, _lateral_movement.len());
            self.details.stats.percent_techniques_command_and_control =
                self.get_percentage(_total, _command_and_control.len());
            self.details.stats.percent_techniques_exfiltration =
                self.get_percentage(_total, _exfiltration.len());
            self.details.stats.percent_techniques_impact =
                self.get_percentage(_total, _impact.len());
            // rollup
    ...
    fn extract_malware(
        ...
        let _malware = items
            .as_object()
            .expect("Malware: Proble Convert Into Oject");
        if _malware.contains_key("revoked") {
            if _malware["revoked"]
                .as_bool()
                .expect("Malaware: Problem Is Revoked Check")
            {
                _is_revoked = true;
            }
        }
        let _malware_id = items["external_references"]
            .as_array()
            .expect("Malware: Problem With External References");
        let _malware_id = _malware_id[0]["external_id"]
            .as_str()
            .expect("Malware: Problem With External ID");
        let _malware_id = _malware_id.to_string();
        let _id = items["id"].as_str().expect("Malware: Problem With UID");
        let _id = _id.to_string();
        let _name = items["name"]
            .as_str()
            .expect("Malware: Problem With Malware Name");
        let _name = _name.to_lowercase()
                         .replace(" ", "-")
                         .replace("/", "");
        let mut _platforms = String::from("");
    ...
    fn extract_tools(
        ...
        if _tools.contains_key("revoked") {
            if _tools["revoked"]
                .as_bool()
                .expect("Tools: Problem Is Revoked Check")
            {
                _is_revoked = true;
            }
        }
        let _tool_id = items["external_references"]
            .as_array()
            .expect("Tools: Problem With External References");
        let _tool_id = _tool_id[0]["external_id"]
            .as_str()
            .expect("Tools: Problem With External ID");
        let _tool_id = _tool_id.to_string();
        let _id = items["id"].as_str().expect("Tools: Problem With UID");
        let _id = _id.to_string();
        let _name = items["name"]
            .as_str()
            .expect("Tools: Problem With Malware Name");
        let _name = _name.to_lowercase()
                         .replace(" ", "-")
                         .replace("/", "");
        let mut _platforms = String::from("");
    ...
    fn extract_adversaries(
        &mut self,
        items: &serde_json::Value,
    ) -> Result<(), Box<dyn std::error::Error>> {
        let mut _is_revoked: bool = false;
        let _adversary = items
            .as_object()
            .expect("Adversary: Proble Convert Into Oject");
        if _adversary.contains_key("revoked") {
            if _adversary["revoked"]
                .as_bool()
                .expect("Adversary: Problem Is Revoked Check")
            {
                _is_revoked = true;
            }
        }
        let _gid = items["external_references"]
            .as_array()
            .expect("Adversary: Problem With External References");
        let _gid = _gid[0]["external_id"]
            .as_str()
            .expect("Adversary: Problem With External ID");
        let _gid = _gid.to_string();
        let _id = items["id"].as_str().expect("Adversary: Problem With UID");
        let _id = _id.to_string();
        let _gname = items["name"]
            .as_str()
            .expect("Adversary: Problem With Technique Name");
        let _gname = _gname.to_lowercase()
                           .replace(" ", "-")
                           .replace("/", "-");
        let mut _aliases = String::from("");
    ...
    fn extract_relationshsip(
        &mut self,
        items: &serde_json::Value,
    ) -> Result<(), Box<dyn std::error::Error>> {
        let _relationship = items
            .as_object()
            .expect("Relationship Problem, Convert To Object");
        let mut _er = EnterpriseRelationship::new();
        _er.id = _relationship["id"].as_str().unwrap().to_string();
        let _sr: &str = _relationship["source_ref"]
            .as_str()
            .expect("Relationship Problem, Convert Source Ref");
        let _sr = _sr.to_string();
        let _tr: &str = _relationship["target_ref"]
            .as_str()
            .expect("Relationship Problem, Convert Target Ref");
    ...
    fn correlate_relationships(&mut self) {
                ...
                if _relationship.target.as_str() == _t.id.as_str() {
                    for _revoked in self.new_revoked_techniques.iter_mut() {
                        if _relationship.source.as_str() == _revoked.id.as_str() {
                            ...
                            self.details.enterprise_revoked_techniques.items.push(_eri);
                        }
                    }
                }
            }
        ...
        self.details.enterprise_revoked_techniques.update();
                    ...
                    for _technique in self.details.breakdown_techniques.platforms.iter() {
                        if _behavior.target.as_str() == _technique.id.as_str() {
                            _adversary
                                ...
                                .techniques
                                .items
                                .push(_technique.tid.clone());
                            _adversary
                                .profile
                                .tactics
                                .items
                                .push(_technique.tactic.clone());
                        }
                    }
                    for _subtechnique in self.details.breakdown_subtechniques.platforms.iter() {
                        if _behavior.target.as_str() == _subtechnique.id.as_str() {
                            _adversary
                                ...
                                .subtechniques
                                .items
                                .push(_subtechnique.tid.clone());
                            _adversary
                                .profile
                                .tactics
                                .items
                                .push(_subtechnique.tactic.clone());
                        }
                    }
                }
            }
        ...
        for _malware in self.details.breakdown_malware.iter_mut() {
            ...
            for _actor in self.details.breakdown_adversaries.iter() {
                for _weapon in _actor.profile.malware.items.iter() {
                    if _malware.name.as_str() == _weapon.as_str() {
                        _malware
                            .profile
                            .adversaries
                            .items
                            .push(_actor.name.clone());
                    }
                }
            }
        ...
        for _tool in self.details.breakdown_tools.iter_mut() {
            ...
            // Tools to Adversaries
            for _actor in self.details.breakdown_adversaries.iter() {
                for _weapon in _actor.profile.tools.items.iter() {
                    if _tool.name.as_str() == _weapon.as_str() {
                        _tool
                            .profile
                            .adversaries
                            .items
